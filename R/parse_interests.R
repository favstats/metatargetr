...
unnest_and_fix_dups <- function(dat, the_list, new_name) {
    ...
    dat %>%
        unnest_longer({{the_list}})  %>%
        ### this part is necessary because sometimes there are duplicate targeting criteria
        mutate(!!rlang::sym(new_name) := ifelse(
            str_ends({{the_list}}, "[:digit:]"),
            str_sub({{the_list}}, 1, nchar({{the_list}})-1),
            {{the_list}})) %>%
        group_by(archive_id) %>%
        distinct(!!rlang::sym(new_name), .keep_all = T) %>%
        ungroup() %>%
        select(-{{the_list}})
}
